...
class _TelaOrcamentoState extends State<TelaOrcamento> {
  ...
  Future<void> _carregarDados() async {
    ...
    if (categoriasJson != null) {
      ...
      _categorias = decodificado.map((item) => {
        'titulo': item['titulo'],
        'icone': _opcoesIcones.firstWhere((iconeConstante) => iconeConstante.codePoint == (item['icone'] as int), orElse: () => Icons.restaurant),
  ...
  Future<void> _salvarDados() async {
    ...
    List<Map<String, dynamic>> paraSalvar = _categorias.map((item) => {
      'titulo': item['titulo'], 'icone': item['icone'].codePoint, 'cor': item['cor'].value, 'gasto': item['gasto'], 'limite': item['limite'],
    }).toList();
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              TextField(controller: valorController, keyboardType: TextInputType.number, decoration: InputDecoration(labelText: 'Valor Total (R\$)', prefixIcon: const Icon(Icons.attach_money), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))),
            ],
          ),
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () {
                ...
                if (novoValor != null && novoValor > 0) { setState(() => _orcamentoTotal = novoValor); _salvarDados(); Navigator.pop(context); }
              },
                    ...
                    children: _opcoesIcones.map((icone) {
                      ...
                      return InkWell(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(color: isSelected ? const Color(0xFF10B981).withOpacity(0.2) : Colors.grey.shade100, border: Border.all(color: isSelected ? const Color(0xFF10B981) : Colors.transparent, width: 2), borderRadius: BorderRadius.circular(12)),
                      ...
                      onPressed: () {
                        if (tituloController.text.isNotEmpty && limiteController.text.isNotEmpty) {
                          ...
                          if (limite != null && limite > 0) {
                            setState(() { _categorias.add({'titulo': tituloController.text, 'icone': iconeSelecionado, 'cor': _opcoesCores[_categorias.length % _opcoesCores.length], 'gasto': 0.0, 'limite': limite}); });
                            _salvarDados(); Navigator.pop(context);
                          }
                        }
                      },
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      IconButton(icon: const Icon(Icons.delete_outline, color: Colors.redAccent), onPressed: () { setState(() { _categorias.removeAt(index); }); _salvarDados(); Navigator.pop(context); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Categoria excluída!'), backgroundColor: Colors.redAccent)); })
                    ],
                  ),
                  const SizedBox(height: 16),
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () {
                        if (tituloController.text.isNotEmpty && limiteController.text.isNotEmpty) {
                          double? limite = double.tryParse(limiteController.text.replaceAll(',', '.'));
                          if (limite != null && limite > 0) {
                            setState(() { _categorias[index]['titulo'] = tituloController.text; _categorias[index]['icone'] = iconeSelecionado; _categorias[index]['limite'] = limite; });
                            _salvarDados(); Navigator.pop(context);
                          }
                        }
                      },
  ...
  void _abrirModalNovoGasto(BuildContext context) {
    if (_categorias.isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Crie uma categoria primeiro!'), backgroundColor: Colors.redAccent)); return; }
                  ...
                  Container(
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        value: categoriaSelecionada, isExpanded: true,
                        items: _categorias.map((cat) { return DropdownMenuItem<String>(value: cat['titulo'], child: Row(children: [Icon(cat['icone'], color: cat['cor'], size: 20), const SizedBox(width: 12), Text(cat['titulo'])])); }).toList(),
                        onChanged: (String? novoValor) { if (novoValor != null) setModalState(() => categoriaSelecionada = novoValor); },
                      ),
                    ),
                  ),
                  const SizedBox(height: 16),
                  TextField(controller: valorController, keyboardType: TextInputType.number, decoration: InputDecoration(labelText: 'Valor do Gasto (R\$)', prefixIcon: const Icon(Icons.attach_money), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))),
                  const SizedBox(height: 32),
                  SizedBox(
                    width: double.infinity,
                    child: ElevatedButton(
                      ...
                      onPressed: () {
                        ...
                        if (valorDigitado == null || valorDigitado <= 0) { setModalState(() => mensagemErro = 'Digite um valor válido.'); return; }
                        setState(() { int index = _categorias.indexWhere((c) => c['titulo'] == categoriaSelecionada); if (index != -1) _categorias[index]['gasto'] += valorDigitado; });
                        _salvarDados(); Navigator.pop(context); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Gasto registrado com sucesso!'), backgroundColor: Color(0xFF10B981)));
                      },
          ...
          children: [
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        ...
                        IconButton(icon: const Icon(Icons.edit, color: Colors.grey, size: 20), onPressed: () => _abrirModalEditarOrcamento(context), tooltip: 'Editar Orçamento Total')
                      ],
                    ),
                    const SizedBox(height: 8),
                    const Text('Disponível para gastar', style: TextStyle(color: Colors.grey)),
                    Text(
                      disponivel >= 0 ? _formatarMoeda(disponivel) : '- ${_formatarMoeda(disponivel.abs())}', 
                      style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: disponivel >= 0 ? const Color(0xFF10B981) : Colors.redAccent)
                    ),
                    const SizedBox(height: 16),
                    LinearProgressIndicator(value: progressoGeral > 1.0 ? 1.0 : progressoGeral, backgroundColor: Colors.grey[100], color: progressoGeral >= 1.0 ? Colors.redAccent : const Color(0xFF10B981), minHeight: 8, borderRadius: BorderRadius.circular(10)),
                    const SizedBox(height: 8),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text('Gasto: ${_formatarMoeda(gastoTotal)}', style: const TextStyle(color: Colors.grey, fontSize: 12)),
                        Text('Orçamento: ${_formatarMoeda(_orcamentoTotal)}', style: const TextStyle(color: Colors.grey, fontSize: 12)),
                      ],
                    ),
                  ],
                ),
            ...
            Row(
              ...
              children: [
                ...
                TextButton(onPressed: () => _abrirModalNovaCategoria(context), child: const Text('+ Nova Categoria', style: TextStyle(color: Color(0xFF10B981), fontWeight: FontWeight.bold)))
              ],
            ),
            const SizedBox(height: 16),

            ..._categorias.asMap().entries.map((entry) {
              int index = entry.key; var categoria = entry.value;
              return Padding(padding: const EdgeInsets.only(bottom: 12.0), child: _construirCategoria(index: index, icone: categoria['icone'], corOriginal: categoria['cor'], titulo: categoria['titulo'], gasto: categoria['gasto'], limite: categoria['limite']));
            }).toList(),
          ],
        ),
      ),
      floatingActionButton: FloatingActionButton(onPressed: () => _abrirModalNovoGasto(context), backgroundColor: const Color(0xFF10B981), elevation: 2, tooltip: 'Adicionar Gasto', child: const Icon(Icons.add, color: Colors.white)),
    );
  }

  Widget _construirCategoria({required int index, required IconData icone, required Color corOriginal, required String titulo, required double gasto, required double limite}) {
        ...
        child: Column(
          children: [
            ListTile(
              ...
              leading: Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: corOriginal.withOpacity(0.1), borderRadius: BorderRadius.circular(10)), child: Icon(icone, color: corOriginal)),
              title: Text(titulo, style: const TextStyle(fontWeight: FontWeight.bold)),
              subtitle: Text('${_formatarMoeda(gasto)} / ${_formatarMoeda(limite)}'),
            ...
            Padding(
              ...
              child: LinearProgressIndicator(value: progresso > 1.0 ? 1.0 : progresso, backgroundColor: Colors.grey.shade100, color: corBarra, minHeight: 4, borderRadius: BorderRadius.circular(10)),
            )
          ],
        ),
      ),
